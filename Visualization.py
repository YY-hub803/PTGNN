...
# 手动设置主刻度之间的分刻度数量
# ax.yaxis.set_minor_locator(AutoMinorLocator(4))  #每两个主刻度之间有四个分刻度
# ax.xaxis.set_minor_locator(AutoMinorLocator(2))  #每两个主刻度之间有两个分刻度
...
def vis_filled_flux(obs_input, pred_input, full_date_range, save_floder=None):
    # 确定时间轴
    ...
    for siteid in columns_nm:
        ...
        # 自动选择刻度间隔 (比手动 MonthLocator 更智能)
        ax.xaxis.set_major_locator(mdates.AutoDateLocator())
        plt.xticks(rotation=30, ha='right')  # 旋转日期标签
        ...
        if save_floder:
            # 组合文件夹路径和文件名
            save_path = os.path.join(save_floder, f"{siteid}_Flux.png")
...
def vis_filled_tp(obs_input, pred_input, full_date_range, save_floder=None):
    # 确定时间轴
    if full_date_range is None:
        if isinstance(obs_input, pd.DataFrame):
            full_date_range = obs_input.index
    ...
    for siteid in columns_nm:
        ...
        # --- 格式美化 ---
        ax.set_title(f"Water Quality Imputation Results - Site: {siteid}", fontsize=14, fontweight='bold')
        ax.set_ylabel("Conc (mg/L)", fontsize=12)
        ax.set_xlabel("Date", fontsize=12)
        # 设置图例 (去掉边框，放在合适位置)
        ax.legend(loc='best', frameon=True, fancybox=True, framealpha=0.9)
        ...
        if save_floder:
            # 组合文件夹路径和文件名
            save_path = os.path.join(save_floder, f"{siteid}_Tp.png")
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"图片已保存至: {save_path}")
            plt.close()
        else:
