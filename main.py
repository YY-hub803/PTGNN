...
dir_output = os.path.join("Q_Yangtze_basin_mask0",dir_model)
...
if 'y_out' in locals():
    print("------------------------ 生成可视化图表 ------------------------------")
    if ny == 2:
        vis.vis_filled_flux(y_true['Dis'], y_out['Dis'], val_date_range, vis_folder)
        vis.vis_filled_tp(y_true['TP'], y_out['TP'], val_date_range, vis_folder)
    else:
        vis.vis_filled_flux(y_true['TP'], y_out['TP'], val_date_range, vis_folder)
